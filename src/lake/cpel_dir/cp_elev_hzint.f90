...
integer*2, dimension (ide,jde)         :: elev1
integer, dimension (ni_cells,nj_cells) :: outflow_matrix_shape

real, dimension (npts)                 :: xdat, wt
real, dimension (npoints)              :: xpt, ypt
...
character(len=100), dimension (ntiles) :: river_input_file, grid_file
...
!           where (interp_out == 0.) elev(:,:,n)= mval_mdl
           ...
           allocate(Interp%horizInterpReals8_type%area_frac_dst(Interp%nxgrid))
           do i = 1, Interp%nxgrid
              ...
              asumi1(idst,jdst)= asumi1(idst,jdst) + Interp%horizInterpReals8_type%area_frac_dst(i)
              asumi2(idst,jdst)= asumi2(idst,jdst) + Interp%horizInterpReals8_type%area_frac_dst(i)**2.
              sumi1(idst,jdst)=  sumi1(idst,jdst)  + Interp%horizInterpReals8_type%area_frac_dst(i)*elev_in(isrc,jsrc)
              ...
              if (Interp%i_src(i) == 1423 .and. Interp%j_src(i) == 1389) &
                  ...
                  Interp%horizInterpReals8_type%area_frac_dst(i)
           enddo
           ...
           do i= 1, Interp%nxgrid
              ...
              sumi2(idst,jdst)= sumi2(idst,jdst) + ((elev_in(isrc,jsrc)- &
                            sumi1(idst,jdst)/asumi1(idst,jdst))**2.)*Interp%horizInterpReals8_type%area_frac_dst(i)
           enddo
...
!              if (idst == 7 .and. jdst == 10 .and. n == 1) then
              if (idst == 1 .and. jdst == 5 .and. n == 1) then
                  write (10,'(3i8,13e20.8)') i, isrc, jsrc, elev_in(isrc,jsrc), arlat(isrc,jsrc), &
                    interp_mask(isrc,jsrc), Interp%horizInterpReals8_type%area_frac_dst(i), sumi1(idst,jdst), sumi2(idst,jdst), &
