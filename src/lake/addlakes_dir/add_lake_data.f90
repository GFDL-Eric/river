...
integer :: n1, np, nwb0, ncasp, varid8, ngdim, ngid, igid, jgid, bgid
integer :: chdim, ivl1, ivl2, jvl1, jvl2, npts_lake_mod
real :: pi, dtr, sum, mval_in, ln1, lf, sum_casp, area_land, scale
real :: sum1, sum2, asum1, asum2, wmax, wmin, travel_thresh
character(len=4)   :: b1
character(len=8)   :: var
character(len=40)  :: var_units
character(len=100) :: fname, glcc_file
logical :: read_lake_frac, scale_area
logical :: cnct_next_all_outlet0, interp_glcc_to_1deg, read_gage_data, cap_lake_depth
...
integer, dimension (ntiles)            :: itw, ite, its, itn, ngage
integer, dimension (ngagemax,ntiles)   :: igage, jgage
integer, dimension (ni_cells,nj_cells) :: outflow_matrix_shape
...
character(len=4), dimension (ngagemax,ntiles) :: bshort
...
real, dimension (ni_cells,nj_cells)    :: out_flow = &
    RESHAPE( (/  8.,   4.,   2., &
       16.,   0.,   1., &
       32.,  64., 128. /), SHAPE(outflow_matrix_shape) )
...
character(len=100), dimension (nvar_glcc) :: lname_glcc

integer, allocatable, dimension (:)     :: ilmod, jlmod, nlmod
integer, allocatable, dimension (:,:,:) :: ga_mask, lake_int, lake_code, tmask

real, allocatable, dimension (:)       :: lat_idx, lon_idx, lfrmod
...
real, allocatable, dimension (:,:,:)   :: lat, lon, tocell, land_frac, cell_area, &
                                          lake_frac, lake_depth, lake_tau, wbd, pwt, &
                                          cnct_next, whole_lake, travel
...
write (6,*) 'test pre file.10'
open (10, file= 'out.add_lake_data', form= 'formatted')
write (6,*) 'test post file.10'

do n= 1,ntiles
   read (24,'(a)') river_input_file(n)
enddo
read (24,'(l1)') read_lake_frac
read (24,'(l1)') cnct_next_all_outlet0
read (24,'(l1)') interp_glcc_to_1deg
read (24,'(l1)') read_gage_data
read (24,'(l1)') cap_lake_depth
read (24,*) travel_thresh
read (24,'(a)') glcc_file
!  occasionally, have to modify a glcc lake frac
read (24,*) npts_lake_mod
if (npts_lake_mod > 0) then
    allocate (ilmod(npts_lake_mod), jlmod(npts_lake_mod), nlmod(npts_lake_mod), lfrmod(npts_lake_mod))
    do l= 1,npts_lake_mod
       read (24,*) nlmod(l), jlmod(l), ilmod(l), lfrmod(l)
    enddo
endif
write (6,*) 'fort.5 data read interior'
    
close (24)

write (6,*) 'fort.5 data read exterior'
...
write (6,*) 'read_gage_data= ', read_gage_data
write (6,*) 'cap_lake_depth= ', cap_lake_depth
write (6,*) 'travel_thresh= ', travel_thresh
...
21  ktr= ktr + 1
      ...
      ngage(n1)= ngage(n1) + 1
      bshort(ngage(n1),n1)= b1
      igage (ngage(n1),n1)= i1
      jgage (ngage(n1),n1)= j1
      go to 21
22  continue
    write (6,*)
    write (6,*) 'number of gages = ', ktr - 1
    close (23)
    ...
    do n= 1,ntiles
       write (6,*) 'tile ', n
       do l= 1,ngage(n)
          write (6,'(a,2i6)') bshort(l,n), igage(l,n), jgage(l,n)
       enddo
    enddo
endif
...
!   write (6,*) 'mval= ', mval_in
   ...
   where (land_frac(:,:,n) == mval_in) land_frac(:,:,n)= mval_mdl
...
!   write (6,*) 'mval= ', mval_in
   ...
   where (cell_area(:,:,n) == mval_in) cell_area(:,:,n)= 0.
...
!   write (6,'(4i6,2f10.2,f15.0)') l, n, i, j, ln1, lat(i+1,j+1,n)
enddo
...
allocate (lake_frac(idp2,jdp2,ntiles))
...
! check to make sure big lake does not occur at partial-land cell
do n= 1,ntiles
   do j= 2,jdp1
      do i= 2,idp1
...
! set lake value to zero at grid cells in antarctic, arctic, and greenland, and also where travel < travel_thresh
!   (the latter so long as lake_frac == 0.)
do n= 1,ntiles
   do j= 2,jdp1
      do i= 2,idp1
         ...
         if (lat(i,j,n) < -60.) then                    ! antarctica
             ...
             if (wbd(i,j,n) > 0.1 .and. travel(i,j,n) <= travel_thresh .and. lake_frac(i,j,n) <= 0.) then
                 wbd(i,j,n)= 0.
             endif
         endif
109      continue
...
! set lake value to zero at manually defined grid cells (in fort.5)  21 dec 2015
if (npts_lake_mod >  0) then
    write (6,*) "***** MANUALLY MODIFY LAKE CELLS *****"
    do l= 1,npts_lake_mod
       n= nlmod(l) ; j= jlmod(l) ; i= ilmod(l)
       write (6,*) l, n, j, i, lfrmod(l)
       wbd(i+1,j+1,n)= lfrmod(l)
    enddo
endif

! double waterbod in amazon region
!do n= 1,ntiles
!   do j= 2,jdp1
!      do i= 2,idp1
!         if ((lon(i,j,n) > 270. .and. lon(i,j,n) < 330.) .and. &
!             (lat(i,j,n) > -20. .and. lat(i,j,n) < 15.)) then
!              wbd(i,j,n)= wbd(i,j,n)*2.
!         endif
!      enddo
!   enddo
!enddo
...
! check that all lake cells have wbd > 0.   11 mar 2014
do n= 1,ntiles
   do j= 2,jdp1
      do i= 2,idp1
         if ( lake_frac(i,j,n) > 0.) then
             if (wbd(i,j,n) == 0. .or. wbd(i,j,n) == mval_mdl) then
                 write (6,'(a,f9.4,a,f9.4,3i6)') &
                 "***WARNING: Lake cell has waterbod= 0, lake_frac= ", &
                lake_frac(i,j,n), ", waterbod= ", wbd(i,j,n), n, j-1, i-1
             else if (wbd(i,j,n) < 0.03) then
                 write (6,'(a,f9.4,a,f9.4,3i6)') &
                 "***WARNING***  Lake cell has small lake fraction, lake_frac= ", &
                   lake_frac(i,j,n), ", waterbod= ", wbd(i,j,n), n, j-1, i-1
             endif
         endif
      enddo
...
! check all waterbod fractions at big-lake outlet cells
   ...
   do j= 2,jdp1
      do i= 2,idp1
         if (lake_frac(i,j,n) > 0. .and. cnct_next(i,j,n) == 0.) then
             ...
             if (wbd(i,j,n) < 0.1) write (6,'(a,f9.4,a,f9.4,3i6)') &
                "***WARNING***  Outlet cell has small lake fraction, lake_frac= ", &
                   lake_frac(i,j,n), ", waterbod= ", wbd(i,j,n), n, j-1, i-1
         endif
      enddo
   enddo
...
! make sure each gage is located on a full-land cell
if (read_gage_data) then
    do n= 1,ntiles
       do l= 1,ngage(n)
          if (bshort(l,n) == 'dumm') go to 180
          i= igage(l,n)+1 ;  j= jgage(l,n)+1
          if (land_frac(i,j,n) /= 1.) then
              write (6,*) "ERROR: gage located on partial-land cell, ", trim(bshort(l,n)), &
                 ", ", land_frac(i,j,n)
              write (6,'(3(a,2x,i3))') "i= ", igage(l,n), ", j= ", jgage(l,n), ", n= ", n
!              stop 180
          endif
180       continue
       enddo
...
!    write (12,*)
!    do j= 60,1,-1
!       write (12,'(i6,71i4)') j, (int(lake_frac(i+1,j+1,3)), i= 120,190)
!    enddo
!    do j= 720,670,-1
!       write (12,'(i6,71i4)') j, (int(lake_frac(i+1,j+1,2)), i= 120,190)
!    enddo

    allocate (tmask(idp2,jdp2,ntiles))
    tmask= tocell
    ...
    do j= 720,670,-1
       write (12,'(i6,71i4)') j, (tmask(i+1,j+1,2), i= 120,190)
    enddo

    do l= 1,nlake_defp1-1
       tmask= tocell
       where (lake_code /= l) tmask= 0
       do n= 1,ntiles
          ivl1= 9999 ;  ivl2= -9999 ;  jvl1= 9999 ;  jvl2= -9999
          do j= 2,jdp1
             do i= 2,idp1
                if (lake_code(i,j,n) == l) then
                    ivl1= min(ivl1,i)
                    ivl2= max(ivl2,i)
                    jvl1= min(jvl1,j)
                    jvl2= max(jvl2,j)
             ...
             enddo
          enddo
          write (12,'(i2,3x,a)') l, trim(lake_name(l))
          if (ivl1 == 9999) then
          write (12,'(5i6)') n, ivl1, ivl2, jvl1, jvl2
              go to 550
          endif
          write (12,'(5i6)') n, ivl1-1, ivl2-1, jvl1-1, jvl2-1
          write (12,'(6x,100i4)') (i-1, i= ivl1,ivl2)
          do j= jvl2,jvl1,-1
             write (12,'(i6,100i4)') j-1, (tmask(i,j,n), i= ivl1,ivl2)
          enddo
550       continue
       enddo
    enddo
...
!  create dimensions (grid_x, grid_y, ngage)
 ...
 ! define character-position dimension
   if (read_gage_data) then
       rcode= NF_DEF_DIM (ncid, 'ngage',  ngage(n), ngdim)
...
!       rcode= NF_DEF_VAR (ncid, 'bname_gage',  NF_CHAR, 1, ngdim,  bgid)
...
!    lake tau data
   ...
   if (read_gage_data) then
       start= 1 ;  count= 1 ;  count(1)= ngage(n)
       rcode= NF_PUT_VARA_DOUBLE (ncid, igid, start, count, real(igage(:,n)))

       start= 1 ;  count= 1 ;  count(1)= ngage(n)
       rcode= NF_PUT_VARA_DOUBLE (ncid, jgid, start, count, real(jgage(:,n)))

       do l= 1,ngage(n)
          start(1)= 1 ;  count(1)= len_bshort
          start(2)= l ;  count(2)= 1
          rcode= NF_PUT_VARA_TEXT (ncid, bgid, start, count, bshort(l,n))
       enddo
   endif
...
deallocate (lat, lon, tocell, land_frac, cell_area, travel)
deallocate (lake_frac, lake_depth, lake_tau)
